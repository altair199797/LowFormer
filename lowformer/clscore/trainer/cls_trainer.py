...
from lowformer.apps.trainer import Trainer
from lowformer.apps.utils import AverageMeter, is_master, sync_tensor
from lowformer.clscore.trainer.utils import accuracy, apply_mixup, label_smooth
from lowformer.models.utils import list_join, list_mean, torch_random_choices
...
class ClsTrainer(Trainer):
        ...
        with torch.no_grad():
            with tqdm(
                ...
                for images, labels in data_loader:
                    ...
                    t.update()
        if timeit:
            print("len total timelist:",len(total_time_list),"bsize:",np.max(np.array(bsizes)))
            print("median time needed:",np.median(np.array(total_time_list))," mean:",np.mean(np.array(total_time_list))) #113 for me 100 for them
        return {
    ...
    def before_step(self, feed_dict: dict[str, any]) -> dict[str, any]:
        ...
        # label smooth
        # print(self.data_provider.n_classes, labels.shape, torch.max(labels))
        # assert False
    ...
    def _train_one_epoch(self, epoch: int) -> dict[str, any]:
        ...
        with tqdm(
            ...
            for images, labels in self.data_provider.train:
                ...
                self.after_step(roundrobin == (bsizemult-1))
                roundrobin = (roundrobin + 1) % bsizemult
    ...
    def train(self, trials=0, save_freq=1, evalnow=False) -> None:
        ...
        if evalnow:
            val_info_dict, model = self.multires_validate(epoch=self.start_epoch)
            model.eval()
            self.save_model(
                    only_state_dict=False,
                    epoch=-1,
                    model_name="evalmodel.pt", spec_model=model)
            print(val_info_dict)

        for epoch in range(self.start_epoch, self.run_config.n_epochs + self.run_config.warmup_epochs):
            
            # print([k["lr"] for k in self.optimizer.param_groups])

            train_info_dict = self.train_one_epoch(epoch)
            # eval
            if not self.no_eval:
                val_info_dict, _ = self.multires_validate(epoch=epoch)
