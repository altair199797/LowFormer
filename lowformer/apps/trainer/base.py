...
from lowformer.apps.data_provider import DataProvider, parse_image_size
from lowformer.apps.trainer.run_config import RunConfig
from lowformer.apps.utils import EMA, dist_barrier, get_dist_local_rank, is_master
from lowformer.models.nn.norm import reset_bn
from lowformer.models.utils import is_parallel, load_state_dict_from_file
...
class Trainer:
    ...
    def save_model(
        ...
        if is_master():
            if checkpoint is None:
                model_saved = spec_model if not spec_model is None else self.network
                if only_state_dict:
                    checkpoint = {"state_dict": model_saved.state_dict()}
                else:
                    checkpoint = {
    ...
    def load_model(self, model_fname=None, imagenetfinetune=False, only_model=False) -> None:
        latest_fname = os.path.join(self.checkpoint_path, "latest.txt")
        ...
        if imagenetfinetune:
            ...
            for key in checkpoint:
                if not "head" in key or not "3" in key: # only last linear layer is removed!
                    tmp[key] = checkpoint[key]
                # else:
                #     print(key, "not included!")
            checkpoint = tmp
            self.network.load_state_dict(checkpoint, strict=False)
        else:
            self.network.load_state_dict(checkpoint["state_dict"], strict=True) # TODO CHANGE STRICT!

        if only_model or imagenetfinetune:
            return
        log = []
        ...
        if "lr_scheduler" in checkpoint:
            # TAKE THE LR FROM CONFIG!!! EDITED BY MORITZ
            checkpoint["lr_scheduler"]["base_lrs"] = self.lr_scheduler.state_dict()["base_lrs"]
    ...
    def multires_validate(
        ...
        for r in eval_image_size:
            ...
            # print("Total time needed for evaluation (s):", time.time()-start_time) # 52 s 47s
    ...
    def after_step(self, stepit=True) -> None:
        if stepit:
        ...
        # update ema
        if self.ema is not None and stepit:
