...
class LowMixer(nn.Module):
    ...
    def forward(self, x):
        # B, C, H, W
        B = torch.jit.annotate(int,x.shape[0])
        # B, _, _, _ = x.shape
        xa = self.pool(x)
        xa = xa.permute(0, 2, 3, 1).view(B, -1, self.dim)
        # _, N, C = xa.shape
        B = torch.tensor(xa.shape[0])
        N = torch.tensor(xa.shape[1])
        C = torch.tensor(xa.shape[2])
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        if self.use_layer_scale:
            ...
            self.layer_scale_1 = torch.ones((dim))
            self.layer_scale_2 = torch.ones((dim))
...
class InceptionTransformer(nn.Module):
    ...
    def _get_pos_embed(self, pos_embed, num_patches_def: int, H:int, W:int):
        if H * W == num_patches_def * num_patches_def:
            return pos_embed
        else:
            return F.interpolate(
                pos_embed.permute(0, 3, 1, 2),
                size=(int(H), int(W)), mode="bilinear").permute(0, 2, 3, 1)
...
def get_iformer(version):
    return eval("iformer_"+version+"()")
