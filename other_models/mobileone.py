...
class MobileOneBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if inference_mode:
            self.reparam_conv = nn.Conv2d(in_channels=in_channels,
                                          ...
                                          groups=groups,
                                          bias=True)
            self.rbr_skip = nn.ModuleList([nn.Identity()])
            self.rbr_conv = nn.ModuleList([nn.Identity()])
            self.rbr_scale = nn.Identity()
        else:
    ...
    def forward(self, x):
        ...
        for ind, i in enumerate(self.rbr_conv):
            out += i(x)
...
def get_mobileone(version):
    
    model = mobileone(variant=version, inference_mode=False)
    model.eval()      
